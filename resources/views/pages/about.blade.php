@extends ('layout')

@section('main')

<main id="contactColumn">
    <div class="contactItem primary">
        <p>LAMP is a graphic design agency based in Maastricht, the Netherlands.
        <span class="blocky"><a href="tel:+310615197033">+31 (0)6 151 97 033</a><br>
        <a href="mailto:info@lampbureau.com" target="_top" class=''>info@lampbureau.com</a><br>
        </p>
        </div>
    <div class="contactItem secondary">
        <p>
            <span class="noTop">
            LAMP Design Bureau<br>
            Minckelersstraat 18<br>
            6211 GX Maastricht</span>
            <span class='blocky'><span class='bold'>KVK</span> 65483820<br>
            <span class='bold'>BTW</span> NL366765899B01</span>
        </p>
       </div>
</div>

@stop

@section('footer')
<footer>

</footer>
@stop